// leaked by t.me/leakstransfer_bot aka BlackLeaks
 ...
 if (process.argv.length < 7){console.log(`命令：\nnode TLS-CHINK.js <url> <time> <rate> <threads> <proxyfile>`); process.exit();}
 const headers = {};
  ...
  Promise.race([axiosPromise, timeoutPromise])
    ...
    .catch((error) => {
      if (error.message === 'sosi penis') {
        ...
        console.log(`信息：${getCurrentTime()} 标题：${extractedTitle} {error.response.status}`);
      } else {
